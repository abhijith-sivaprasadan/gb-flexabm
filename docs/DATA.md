...
Elexon wholesale Market Index and FUELHH acquisition is now implemented, with 402 requests for each dataset covering only training years. The [full audit](MARKET_DATA.md) supersedes the earlier January 1 probes: no APXMIDP price year has complete coverage. Negative prices and signed generation/interconnector flows are retained; zero-volume index periods fail target usability. Auxiliary FUELHH settlement-label defects are reported while preserving primary UTC timestamps. No gaps are filled, and DUKES/fleet integration and demand/generation boundary reconciliation remain pending. The [full checklist](HISTORICAL_DATA.md) specifies all remaining inputs. Prior 2024 demand exposure remains recorded; the draft protocol has not been frozen or widened.
