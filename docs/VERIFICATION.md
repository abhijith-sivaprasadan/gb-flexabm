...
## S2 market-source evidence milestone (31 August 2026)

- Local Python 3.12: **140 tests pass**, Ruff lint/format pass and mypy passes for 17 source files. The 14 upstream Matplotlib/Pyparsing warnings remain visible. No test depends on live API availability.
- Actual acquisition completed **402 MID and 402 FUELHH responses**, restricted to the draft 2013–2018 training years. All cached bytes and recorded coverage were reverified with the current adapter. The public [price audit](reference/training-market-mid.json) and [generation audit](reference/training-market-fuelhh.json) publish response hashes and coverage, not raw observations. Their own digests detect accidental edits, not forgery or scientific invalidity.
- **No training year passes full APXMIDP coverage.** The [market-data findings](MARKET_DATA.md) document absent early years, partial 2016, gaps in 2017/2018 and zero-volume periods. FUELHH gaps, changing categories and inconsistent auxiliary settlement labels are retained as explicit limitations. No prices were interpolated, no provider or price product substituted, and no split window changed.
- New regressions cover training-only request bounds, negative prices and signed flows, zero/non-finite volume, empty/missing intervals, UTC half-hour grids, DST, inclusive query ends, revision conflicts, energy-conserving hourly means, retained rejected responses and modified cache/report rejection.
- Real Streamlit AppTest verifies the audit display and removal of a tampered report. Browser inspection confirms both six-year tables, timestamps, source links and explicit **INCOMPLETE** cells. Small audit tables use accessible HTML with dark headers/cells rather than subtle canvas checkboxes. A targeted DOM-text contrast check reported no issues for 181 sampled workbench and 132 portfolio elements; this is not a full accessibility certification.
- The companion portfolio case study, README, project discovery summary and generated academic pages were updated. Local checks passed for 92 HTML pages, 65 JavaScript files, 23 JSON files and the 16 generated pages (five tracks/eight skill dossiers). Check the live workflows for publication status.
- The preserved v0.1 reference and both synthetic dispatch benchmarks still pass integrity verification. No historical fitting, parameter lock, no-fit evaluation or independent review has occurred. Full weather acquisition/conversion and the other historical inputs remain pending; this milestone does **not** complete S2–S5.

## S2 public-input acquisition/extraction milestone (31 August 2026)

- Local Python 3.12: **165 tests pass**, Ruff lint/format pass and mypy passes for 18 source files. The 14 existing Matplotlib/Pyparsing warnings remain visible. The complete suite includes real Streamlit AppTest/CLI replay; no GUI or scientific-engine code changed in this milestone.
- The [metadata-only inventory](reference/public-inputs-inventory.json) was generated by rehashing originals and extraction outputs. A final independent invocation confirmed 175 acquired snapshots, 465,042,005 raw bytes, and no pending extraction among those snapshots. The two inaccessible CSV URLs are preserved as exceptions; the corresponding public API returned 304/304 administrative-strike-price records.
- Extraction covers 25 CSV, 135 PDF (3,688 pages), nine XLSX, five HTML and one JSON files. The large NESO history uses row-indexed JSONL; quoted CSV newlines, workbook formula/cell identity, HTML spans and PDFs without content streams have authored regression tests. PDF sample visual checks are documented in [PUBLIC_INPUTS.md](PUBLIC_INPUTS.md), not claimed as all-page table certification.
- Acquisition regressions reject unapproved hosts, credential-bearing URLs, unsafe identifiers, HTML error pages, altered cached bytes and invalid/oversized file limits. Signed redirect query strings are excluded from receipts. Raw publisher observations, contact details and private attachments are not committed.
- IMRP has 89,064 native hourly-period rows beginning 30 June 2016. Training-price QA rejects non-finite values and preserves negative prices in fixtures; absent years and duplicate keys cannot become complete coverage. No APXMIDP substitution, UTC interpretation, study-window change or fitting occurred. Later-period mechanical extraction/current revisions are recorded as prior exposure.
- Scientific smoke checks and integrity checks for the original v0.1 experiment and both synthetic benchmarks pass unchanged. This is acquisition/extraction evidence, not empirical S2–S5 completion.
- Companion portfolio checks pass for 92 HTML pages, 65 JavaScript files, 23 JSON files and 16 generated academic pages. Browser navigation reached the case study from the landing-page Case study link. A targeted light-theme contrast probe reported no failures for 119 rendered text elements and no horizontal page overflow; this is not a full accessibility certification. The README, case study and discovery/generated summaries have been updated together. Consult live workflows for remote publication status.
